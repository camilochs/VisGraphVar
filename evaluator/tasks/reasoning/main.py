...
def evaluation_reasoning(benchmark_path, models, prompts_type, template_prompts):
    task = "reasoning"
    total_executions = 0
    prompt = {}
            ...
            if ".png" in file:
                for model_name, prompt_type in itertools.product(models, prompts_type):
                    ...
                    if not os.path.exists(path_result_model):
                        print(f"Start: {path_result_model}")
                        
                        match = re.search(r"(\d+-\d+)", file)
                        start_node, end_node = tuple(map(str, match.group(1).split('-')))
                        
                        prompt_start_end_nodes = prompt[path_prompt].replace("{start_node}", start_node)
                        prompt_start_end_nodes = prompt_start_end_nodes.replace("{end_node}", end_node) 
                        output_llm = execute(model_name, prompt_start_end_nodes, path_image)
                        while not output_llm:
                            output_llm = execute(model_name, prompt_start_end_nodes, path_image)
                        #time.sleep(1)
